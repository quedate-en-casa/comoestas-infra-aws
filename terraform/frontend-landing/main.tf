...
resource "aws_s3_bucket" "s3_bucket_comoestaslanding" {
  bucket = var.s3_bucket_name
  acl    = var.s3_bucket_acl
  tags   = var.s3_bucket_tags
}

resource "aws_cloudfront_distribution" "cf_distribution_comoestaslanding" {
  origin {
    domain_name = aws_s3_bucket.s3_bucket_comoestaslanding.bucket_regional_domain_name
